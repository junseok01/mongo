...
public class ScoreController {
    ...
    public String read(@RequestParam("id") String id,@RequestParam("action") String action, Model model){
            ScoreDTO dto = service.findById("id",id);
            String view ="";
    ...
    @GetMapping("/read2")
    public String read2(@RequestParam("id") String id,@RequestParam("action") String action, Model model){
        ScoreDTO dto = service.findById(id);
        String view ="";
        if(action.equals("READ")){
            view="mongo/mongo_detail";
        }else{
